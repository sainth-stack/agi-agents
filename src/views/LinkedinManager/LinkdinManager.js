...
const LinkdinManager = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("post-type", option); // Selected option
      formData.append("yt-url", websiteUrl); // Website URL
      // formData.append("topic", topic); // Topic

      // Make API request to generate content
      const response = await axios.post(
        "http://127.0.0.1:8000/api/generate_linkedin_post/",
        formData,
        {
          headers: {
